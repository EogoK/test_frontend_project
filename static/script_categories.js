...
async function loadCategories() {
    ...
    categories.forEach((category, index) => {
        //var category = i;
        let categoryItem = categoryItemTemplate.content.cloneNode(true);
        const categoryItemImg = categoryItem.querySelector('.category-item__icon');
        const categoryItemName = categoryItem.querySelector('.category-item__name');
        
        categoryItemImg.src = catpics[category];
        categoryItemName.textContent = category;
        categoryItem.querySelector('.category-item').dataset.id = index;
        categoryItems.appendChild(categoryItem);

        categoryItem = categoryItems.querySelector(`.category-item[data-id="${index}"]`);
        const goCategoryButton = categoryItem.querySelector(".category-item__button[data-add]");
        //goCategoryButton.addEventListener('click', () => goCategoryListener(category));
        goCategoryButton.addEventListener('click', () => {window.location.href = "./subcategs?category=" + category});
    })
}
loadCategories()

function goCategoryListener(category) {
    console.log(category)
    window.location.href = "./subcategs?category=" + category
}
