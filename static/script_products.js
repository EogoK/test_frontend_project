...
async function loadItems(category, subcategory) {
    const response = await fetch('/static/products.json');
    const data = await response.json();

    var products = Object.values(data[category][subcategory])
    products.forEach((product, index) => {
        ...
        foodItemImg.src = product["Ссылка-Фото"];
        foodItemName.textContent = product["Наименование"];
        var prices = Object.values(product["Магазины"])

        foodItemPrice.textContent = formatter.format(prices.min());
    ...
    //     foodItemPrice.textContent = formatter.format(prices.min());
    //     foodItem.querySelector('.food-item').dataset.id = i;
    //     foodItems.appendChild(foodItem);

    //     foodItem = foodItems.querySelector(`.food-item[data-id="${i}"]`);
    //     const addItemButton = foodItem.querySelector(".food-item__button[data-add]");
    //     addItemButton.addEventListener('click', () => addItemListener(foodItem, index));
    //     const removeItemButton = foodItem.querySelector('.food-item__button[data-remove]');
    //     removeItemButton.addEventListener('click', () => removeItemListener(foodItem, index));
...
loadItems(category, subcategory)
